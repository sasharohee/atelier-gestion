-- Script de diagnostic et correction des données (version UUID corrigée)
...
-- 2. CORRECTION : Créer des données de test avec user_id (UUID)
...
DO $$
DECLARE
    test_user_id UUID := '13d6e91c-8f4b-415a-b165-d5f8b4b0f72a'::UUID;
BEGIN
    ...
    -- Créer une catégorie de test avec user_id (UUID)
    ...
    -- Créer les marques de test avec user_id (UUID)
...
SELECT id, name, description, user_id FROM public.device_categories WHERE user_id = '13d6e91c-8f4b-415a-b165-d5f8b4b0f72a'::UUID;

SELECT 'Marques créées:' as info;
SELECT id, name, description, user_id FROM public.device_brands WHERE user_id = '13d6e91c-8f4b-415a-b165-d5f8b4b0f72a'::UUID ORDER BY name;
...
WHERE db.user_id = '13d6e91c-8f4b-415a-b165-d5f8b4b0f72a'::UUID;
