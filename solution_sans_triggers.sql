-- Solution sans triggers pour le suivi des réparations
-- Contourne complètement les problèmes d'authentification
...
-- 3. Désactiver temporairement les triggers problématiques
...
-- 5. Créer une réparation de test en contournant les triggers
INSERT INTO repairs (
    client_id, 
    device_id,
    status, 
    ...
    total_price, 
    is_paid,
    user_id  -- Ajouter user_id si nécessaire
) 
SELECT 
    c.id,
    NULL,
    'in_progress',
    'Test de réparation sans triggers',
    'Problème de test simple',
    'REP-20250829-1296',
    50.00,
    false,
    (SELECT id FROM users LIMIT 1)  -- Prendre le premier utilisateur si disponible
FROM clients c
...
-- 7. Vérifier que les vues sont créées
...
-- 8. Tester la vue repair_tracking_view
...
-- 9. Tester la vue repair_history_view
...
-- 10. Vérifier que la réparation existe
