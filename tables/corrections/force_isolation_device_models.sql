-- Script pour forcer l'isolation des device_models
-- Ce script va supprimer les données problématiques et recréer une isolation stricte
...
-- 2. Obtenir le workshop_id actuel
DO $$
DECLARE
    v_workshop_id UUID;
...
BEGIN
    ...
    IF v_workshop_count = 0 THEN
        ...
        INSERT INTO system_settings (key, value, user_id, created_at, updated_at)
        VALUES (
            'workshop_id', 
...
CREATE OR REPLACE FUNCTION test_force_isolation()
RETURNS TABLE (
    test_name TEXT,
    status TEXT,
    details TEXT
...
-- 15. Fonction pour vérifier l'état final
CREATE OR REPLACE FUNCTION verify_force_isolation()
RETURNS TABLE (
    verification TEXT,
    result TEXT
) AS $$
...
-- 16. Afficher le statut final
SELECT 'Script force_isolation_device_models.sql exécuté avec succès' as status;
SELECT 'Toutes les données ont été supprimées et l''isolation stricte a été mise en place' as message;
