...
-- 2. CRÉER UN WORKSHOP_ID UNIQUE PAR UTILISATEUR
...
BEGIN
    ...
    -- Mettre à jour ou créer le workshop_id dans system_settings
    INSERT INTO system_settings (key, value, user_id, category, created_at, updated_at)
    VALUES ('workshop_id', workshop_id_value, current_user_id, 'general', NOW(), NOW())
    ON CONFLICT (key) 
    DO UPDATE SET 
        value = workshop_id_value,
        user_id = current_user_id,
        updated_at = NOW();
    
    RAISE NOTICE '✅ Workshop ID créé: % pour utilisateur: %', workshop_id_value, current_user_email;
END $$;

-- 3. METTRE À JOUR TOUTES LES DONNÉES AVEC LE NOUVEAU WORKSHOP_ID
...
BEGIN
    ...
    current_user_id := auth.uid();
    SELECT value INTO current_workshop_id FROM system_settings WHERE key = 'workshop_id' LIMIT 1;
...
-- 5. CRÉER DES POLITIQUES RLS STRICTES PAR USER_ID
...
-- 6. CRÉER DES TRIGGERS POUR DÉFINIR AUTOMATIQUEMENT USER_ID
...
BEGIN
    ...
    VALUES ('Test Isolation', 'Forcee', 'other', 2024)
...
SELECT 'ISOLATION FORCÉE TERMINÉE' as status;
