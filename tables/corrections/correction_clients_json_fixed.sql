-- Correction de l'erreur json_agg dans la fonction RPC
-- Date: 2024-01-24
-- Solution pour corriger l'erreur de json_agg
...
-- Supprimer les anciennes fonctions qui causent des erreurs
DROP FUNCTION IF EXISTS get_my_clients();
DROP FUNCTION IF EXISTS get_my_clients_json();

-- ========================================
-- 3. CRÉER UNE FONCTION RPC SIMPLE ET FONCTIONNELLE
-- ========================================

-- Fonction RPC simple pour récupérer les clients (sans problèmes de types)
CREATE OR REPLACE FUNCTION get_my_clients_simple()
RETURNS TABLE (
    id UUID,
    first_name TEXT,
    last_name TEXT,
...
GRANT EXECUTE ON FUNCTION get_my_clients_simple() TO authenticated;
GRANT EXECUTE ON FUNCTION get_my_clients_simple() TO anon;

-- ========================================
-- 4. CRÉER UNE FONCTION JSON CORRIGÉE
-- ========================================

-- Fonction RPC JSON corrigée (sans json_agg problématique)
...
-- Tester la fonction table simple
SELECT * FROM get_my_clients_simple() LIMIT 3;
...
BEGIN
    ...
    RAISE NOTICE '🔧 FONCTIONS RPC JSON CORRIGÉES !';
    RAISE NOTICE '✅ Erreur json_agg corrigée';
    RAISE NOTICE '✅ Fonction get_my_clients_simple() créée';
    RAISE NOTICE '✅ Fonction get_my_clients_json() corrigée';
    ...
    RAISE NOTICE '- get_my_clients_simple() pour récupérer les clients (table)';
    ...
    RAISE NOTICE '1. Utilisez get_my_clients_json() pour récupérer les clients (JSON)';
    RAISE NOTICE '2. Utilisez get_my_clients_simple() pour récupérer les clients (table)';
    RAISE NOTICE '3. Utilisez add_client_rpc(...) pour ajouter des clients';
    RAISE NOTICE '4. Les fonctions RPC contournent les problèmes RLS';
    RAISE NOTICE '';
    RAISE NOTICE '⚠️ IMPORTANT:';
    RAISE NOTICE '- L''erreur json_agg est corrigée avec une CTE';
    RAISE NOTICE '- Les types de données sont gérés avec COALESCE';
    RAISE NOTICE '- Les fonctions RPC sont sécurisées avec SECURITY DEFINER';
    RAISE NOTICE '';
    RAISE NOTICE '✅ FONCTIONS RPC JSON CORRIGÉES ET FONCTIONNELLES !';
END $$;
