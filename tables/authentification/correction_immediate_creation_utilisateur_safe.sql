-- Correction immédiate pour la création d'utilisateurs (version sûre)
-- Script qui gère les contraintes de clé étrangère
...
-- 6.5. Corriger le trigger pour gérer workshop_id
...
AS $$
BEGIN
  -- Créer automatiquement un profil utilisateur avec workshop_id par défaut
  ...
  VALUES (
    ...
    NEW.last_name,
    NEW.email,
    COALESCE(NEW.workshop_id, 'default-workshop'),
    NOW(),
    NOW()
  )
