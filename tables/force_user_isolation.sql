...
-- Ce script s'assure que toutes les données sont correctement isolées

-- 1. Vérifier que les colonnes user_id existent
DO $$
DECLARE
...
BEGIN
    FOR current_table IN SELECT unnest(ARRAY['clients', 'devices', 'repairs', 'sales', 'appointments', 'parts', 'products', 'services'])
    LOOP
        SELECT EXISTS (
            SELECT 1 FROM information_schema.columns 
            WHERE table_name = current_table AND column_name = 'user_id'
        ) INTO column_exists;
        
        IF NOT column_exists THEN
            missing_columns := array_append(missing_columns, current_table);
        END IF;
    END LOOP;
    
    IF array_length(missing_columns, 1) > 0 THEN
        RAISE EXCEPTION 'Colonnes user_id manquantes dans les tables: %', array_to_string(missing_columns, ', ');
    END IF;
END $$;

-- 2. S'assurer que toutes les données ont un user_id
-- Assigner les données existantes au premier utilisateur admin ou au premier utilisateur disponible
...
-- 3. Forcer la contrainte NOT NULL sur user_id
...
-- 6. Créer des politiques RLS STRICTES qui forcent l'isolation
-- Clients
...
SELECT 
    'Isolation forcée terminée' as status,
...
-- 10. Vérifier qu'il n'y a pas de données orphelines
SELECT 
