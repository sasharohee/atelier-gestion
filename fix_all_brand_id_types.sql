-- ============================================================================
-- SCRIPT DE CORRECTION COMPLÈTE DE TOUS LES TYPES BRAND_ID
-- ============================================================================
-- Ce script corrige définitivement tous les conflits de type UUID vs TEXT
-- pour toutes les tables qui référencent device_brands
-- ============================================================================

SELECT '=== CORRECTION COMPLÈTE DE TOUS LES TYPES BRAND_ID ===' as section;

-- 1. VÉRIFIER LES TYPES ACTUELS DE TOUTES LES COLONNES BRAND_ID
-- ============================================================================
SELECT 
    table_name,
    column_name,
...
-- 2. IDENTIFIER TOUTES LES VUES QUI DÉPENDENT DE DEVICE_BRANDS
-- ============================================================================
...
-- 3. IDENTIFIER TOUTES LES CONTRAINTES QUI DÉPENDENT DE DEVICE_BRANDS
-- ============================================================================
...
-- 4. SAUVEGARDER TOUTES LES DONNÉES ET DÉFINITIONS
-- ============================================================================
SELECT '=== SAUVEGARDE COMPLÈTE DES DONNÉES ===' as section;

-- Créer des tables de sauvegarde pour toutes les tables concernées
CREATE TABLE IF NOT EXISTS device_brands_backup_complete AS 
SELECT * FROM device_brands WHERE false;

CREATE TABLE IF NOT EXISTS device_models_backup_complete AS 
SELECT * FROM device_models WHERE false;

CREATE TABLE IF NOT EXISTS brand_categories_backup_complete AS 
SELECT * FROM brand_categories WHERE false;

-- Vider les tables de sauvegarde
DELETE FROM device_brands_backup_complete;
DELETE FROM device_models_backup_complete;
DELETE FROM brand_categories_backup_complete;

-- Copier les données existantes
INSERT INTO device_brands_backup_complete 
SELECT * FROM device_brands;

INSERT INTO device_models_backup_complete 
SELECT * FROM device_models;

INSERT INTO brand_categories_backup_complete 
SELECT * FROM brand_categories;

SELECT 
    (SELECT COUNT(*) FROM device_brands_backup_complete) as device_brands_backup_count,
    (SELECT COUNT(*) FROM device_models_backup_complete) as device_models_backup_count,
    (SELECT COUNT(*) FROM brand_categories_backup_complete) as brand_categories_backup_count;
...
CREATE TEMP TABLE IF NOT EXISTS view_definition_backup_complete (
    view_name TEXT,
    view_definition TEXT
);

-- Supprimer l'ancienne définition si elle existe
DELETE FROM view_definition_backup_complete WHERE view_name = 'brand_with_categories';

-- Insérer la définition actuelle de la vue
INSERT INTO view_definition_backup_complete (view_name, view_definition)
SELECT 
...
-- 6. SUPPRIMER TEMPORAIREMENT TOUTES LES VUES ET CONTRAINTES
-- ============================================================================
SELECT '=== SUPPRESSION TEMPORAIRE DE TOUTES LES DÉPENDANCES ===' as section;

-- Supprimer la vue brand_with_categories
DROP VIEW IF EXISTS public.brand_with_categories CASCADE;

-- Supprimer toutes les contraintes de clé étrangère qui dépendent de device_brands.id
...
SELECT '✅ Toutes les vues et contraintes supprimées temporairement' as status;

-- 7. CORRIGER LE TYPE DE LA COLONNE ID DANS DEVICE_BRANDS
-- ============================================================================
...
-- Créer un index unique sur id seul pour les contraintes de clé étrangère
CREATE UNIQUE INDEX IF NOT EXISTS device_brands_id_unique ON public.device_brands (id);

SELECT '✅ Type de device_brands.id corrigé de UUID vers TEXT' as status;

-- 8. CORRIGER LE TYPE DE BRAND_ID DANS BRAND_CATEGORIES
-- ============================================================================
...
-- 9. CORRIGER LE TYPE DE BRAND_ID DANS DEVICE_MODELS
-- ============================================================================
SELECT '=== CORRECTION DU TYPE BRAND_ID DANS DEVICE_MODELS ===' as section;
...
-- Si device_models.brand_id est de type UUID, le changer en TEXT
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
    ...
    ELSE
        RAISE NOTICE '✅ Type de device_models.brand_id déjà correct (TEXT)';
    END IF;
END $$;

-- 10. RECRÉER TOUTES LES CONTRAINTES DE CLÉ ÉTRANGÈRE
-- ============================================================================
SELECT '=== RECRÉATION DE TOUTES LES CONTRAINTES DE CLÉ ÉTRANGÈRE ===' as section;
...
SELECT '✅ Toutes les contraintes de clé étrangère recréées' as status;

-- 11. RECRÉER LA VUE BRAND_WITH_CATEGORIES
-- ============================================================================
...
-- 12. RECRÉER TOUTES LES FONCTIONS RPC AVEC LES BONS TYPES
-- ============================================================================
SELECT '=== RECRÉATION DE TOUTES LES FONCTIONS RPC ===' as section;
...
-- 13. ACCORDER TOUTES LES PERMISSIONS
-- ============================================================================
SELECT '=== CONFIGURATION DE TOUTES LES PERMISSIONS ===' as section;
...
-- 14. VÉRIFICATION FINALE COMPLÈTE
-- ============================================================================
SELECT '=== VÉRIFICATION FINALE COMPLÈTE ===' as section;

-- Vérifier tous les types après correction
SELECT 
    table_name,
    column_name,
    data_type
FROM information_schema.columns 
WHERE column_name LIKE '%brand_id%' OR (table_name = 'device_brands' AND column_name = 'id')
AND table_schema = 'public'
ORDER BY table_name, column_name;
...
-- Vérifier que toutes les contraintes de clé étrangère sont recréées
SELECT 
    tc.table_name,
    tc.constraint_name,
    tc.constraint_type,
...
SELECT '✅ Correction complète de tous les types brand_id terminée avec succès' as status;
