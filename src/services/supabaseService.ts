...
export const clientService = {
  ...
  async create(client: Omit<Client, 'id' | 'createdAt' | 'updatedAt'>) {
    try {
      ...
      try {
        const { data: rpcResult, error: rpcError } = await supabase.rpc('create_client_smart', {
        ...
        if (rpcError) {
          console.warn('⚠️ Erreur RPC, fallback vers méthode directe:', rpcError);
          throw rpcError; // Continuer vers la méthode de fallback
        }
        
        console.log('📥 CLIENT SERVICE - RÉPONSE RPC:', rpcResult);
        
        if (rpcResult.success) {
          if (rpcResult.action === 'existing_client_found') {
            // Client existant trouvé, retourner les données existantes
            console.log('ℹ️ Client existant trouvé:', rpcResult.client_data);
            return handleSupabaseSuccess(rpcResult.client_data);
          } else if (rpcResult.action === 'client_created') {
            // Nouveau client créé, récupérer les données complètes
            ...
            const convertedData = {
              ...
              vatNumber: fullData.vat_number,
              sirenNumber: fullData.siren_number,
              countryCode: fullData.country_code,
              
              // Nouveaux champs pour l'adresse détaillée
              addressComplement: fullData.address_complement,
              region: fullData.region,
              postalCode: fullData.postal_code,
              city: fullData.city,
              
              // Nouveaux champs pour l'adresse de facturation
              billingAddressSame: fullData.billing_address_same,
              billingAddress: fullData.billing_address,
              billingAddressComplement: fullData.billing_address_complement,
              billingRegion: fullData.billing_region,
              billingPostalCode: fullData.billing_postal_code,
              billingCity: fullData.billing_city,
              
              // Nouveaux champs pour les informations complémentaires
              accountingCode: fullData.accounting_code,
              cniIdentifier: fullData.cni_identifier,
              attachedFilePath: fullData.attached_file_path,
              internalNote: fullData.internal_note,
              
              // Nouveaux champs pour les préférences
              status: fullData.status,
              smsNotification: fullData.sms_notification,
              emailNotification: fullData.email_notification,
              smsMarketing: fullData.sms_marketing,
              emailMarketing: fullData.email_marketing,
              
              createdAt: fullData.created_at,
              updatedAt: fullData.updated_at
            };
            
            console.log('✅ CLIENT CRÉÉ AVEC SUCCÈS (RPC):', convertedData);
            return handleSupabaseSuccess(convertedData);
          }
        }
      } catch (rpcError) {
        console.log('⚠️ Méthode RPC non disponible, utilisation de la méthode directe');
      }
      
      // Méthode de fallback : création directe (plus d'erreur 409 grâce au trigger supprimé)
      console.log('📤 CLIENT SERVICE - UTILISATION DE LA MÉTHODE DIRECTE');
