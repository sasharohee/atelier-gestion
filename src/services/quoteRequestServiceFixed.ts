// Service corrigé pour les demandes de devis - utilise Supabase avec gestion d'erreurs améliorée
import { supabase } from '../lib/supabase';

export class QuoteRequestServiceFixed {
  ...
  static async getTechnicianByCustomUrl(customUrl: string): Promise<{
    ...
    try {
      ...
      if (!customUrlData) {
        console.error('Aucune URL personnalisée trouvée');
        return null;
      }
  ...
  static async createQuoteRequest(requestData: any): Promise<any> {
    try {
      ...
      if (numberError) {
        ...
        const fallbackNumber = 'QR-' + Date.now();
        requestData.request_number = fallbackNumber;
      } else {
  ...
  // Récupérer les statistiques des demandes avec fallback
  static async getQuoteRequestStats(technicianId: string): Promise<any> {
    try {
      ...
      if (error) {
        console.error('Erreur récupération stats via RPC:', error);
  ...
  // Gérer les URLs personnalisées avec fallback
...
export const quoteRequestServiceFixed = QuoteRequestServiceFixed;
