-- Script de correction pour la structure des tables des demandes de devis
-- Ce script gère les cas où les tables existent déjà avec des structures différentes
...
-- 2. Créer la table technician_custom_urls si elle n'existe pas
...
-- 3. Supprimer la table quote_requests si elle existe déjà (pour repartir proprement)
DROP TABLE IF EXISTS public.quote_requests CASCADE;

-- 4. Créer la table quote_requests avec la structure correcte
...
-- 5. Ajouter la contrainte unique sur request_number
ALTER TABLE public.quote_requests ADD CONSTRAINT unique_request_number UNIQUE (request_number);

-- 6. Supprimer la table quote_request_attachments si elle existe déjà
DROP TABLE IF EXISTS public.quote_request_attachments CASCADE;

-- 7. Créer la table quote_request_attachments
...
-- 8. Créer les fonctions (les recréer même si elles existent)
...
-- 9. Activer RLS (Row Level Security) pour toutes les tables
...
-- 10. Supprimer les politiques existantes (si elles existent) et les recréer
...
-- 12. Créer les index pour améliorer les performances
...
-- 13. Créer la fonction trigger pour updated_at
...
-- 14. Créer les triggers pour mettre à jour automatiquement updated_at
