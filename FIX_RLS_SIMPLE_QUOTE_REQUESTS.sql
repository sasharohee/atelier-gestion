-- Correction simple des politiques RLS pour les demandes de devis
-- Ce script corrige les erreurs 403 Forbidden de manière sûre

-- 1. Vérifier d'abord si les tables existent
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'technician_custom_urls' AND table_schema = 'public') THEN
        RAISE NOTICE 'Table technician_custom_urls n''existe pas. Exécutez d''abord CREATE_QUOTE_TABLES.sql';
        RETURN;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'quote_requests' AND table_schema = 'public') THEN
        RAISE NOTICE 'Table quote_requests n''existe pas. Exécutez d''abord CREATE_QUOTE_TABLES.sql';
        RETURN;
    END IF;
    
    RAISE NOTICE 'Tables trouvées, procédure de correction...';
END $$;

-- 2. Supprimer les anciennes politiques qui peuvent causer des conflits
...
-- 3. Supprimer aussi les politiques de test si elles existent
DROP POLICY IF EXISTS "test_technician_custom_urls_all" ON technician_custom_urls;
DROP POLICY IF EXISTS "test_quote_requests_all" ON quote_requests;
DROP POLICY IF EXISTS "test_quote_request_attachments_all" ON quote_request_attachments;
...
-- 6. Créer des politiques de test temporaires (plus permissives)
-- Ces politiques permettent toutes les opérations pour tester
...
-- 8. Test de création d'URL (à exécuter manuellement pour tester)
-- Remplacez 'YOUR_USER_ID' par l'ID de votre utilisateur
-- INSERT INTO technician_custom_urls (technician_id, custom_url, is_active)
-- VALUES ('YOUR_USER_ID', 'test-url', true);

-- 9. Commentaires sur les corrections
...
DO $$
BEGIN
    RAISE NOTICE 'Correction RLS terminée. Les politiques de test permettent toutes les opérations.';
    RAISE NOTICE 'Testez la création d''URLs maintenant.';
    RAISE NOTICE 'Si cela fonctionne, vous pouvez supprimer les politiques de test plus tard.';
END $$;
